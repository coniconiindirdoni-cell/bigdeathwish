...
app.get('/api/v1/status',async(_q,r)=>{
  try{
    await pool.query('SELECT 1');
    r.json({ok:true,ready:!restoring,generation,restoring,lastBackup,lastRestore});
  }catch(e){
    r.status(503).json({ok:false,ready:false,error:e.message,generation});
  }
});
...
app.post('/api/v1/query',async(req,res)=>{
  try{
    ...
    if(typeof text!=='string'||!text.trim())
      return res.status(400).json({ok:false,error:'text zorunlu'});
    const out=await pool.query(text,params);
    res.json({ok:true,rows:out.rows,rowCount:out.rowCount});
  }catch(e){
...
app.post('/api/v1/logs',async(req,res)=>{
  try{
    ...
    await pool.query(
      'INSERT INTO logs(service_name,level,message,file_name,user_id,server_id,metadata) VALUES($1,$2,$3,$4,$5,$6,$7)',
      [service,level,message,fileName,userId,serverId,metadata]
    );

    console.log("✅ LOG VERİTABANINA KAYDEDİLDİ");
...
app.post('/api/v1/backup',async(_q,r)=>{
  try{
    r.json({ok:true,...await backup()});
  }catch(e){
    r.status(500).json({ok:false,error:e.message});
  }
});

app.post('/api/v1/restore',async(req,r)=>{
  try{
    r.json(await restoreIfEmpty(req.body?.force===true));
  }catch(e){
    r.status(500).json({ok:false,error:e.message});
  }
});
...
(async()=>{
  await pool.query('SELECT 1');
  await restoreIfEmpty(false);
  app.listen(PORT,()=>console.log(`database-service ${PORT}`));
  const t=setInterval(()=>backup().catch(e=>console.error('Saatlik backup:',e.message)),60*60*1000);
  t.unref?.();
})();
